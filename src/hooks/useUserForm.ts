...
const useUserForm = () => {
    const [values, setValues] = useLocalStorage<FormValues>(LOCAL_STORAGE_KEY, {
        username: "",
        email: ""
    }, 300);
    ...
    const handleSubmit = async (event :React.SubmitEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const serverError = error instanceof Error ? error.message : (typeof error === "string" ? error : String(error));
